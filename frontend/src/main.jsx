  ...
  return <div className="min-h-screen overflow-hidden">
    ...
    <header className="sticky top-0 z-30 border-b border-white/10 bg-ink/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4 lg:px-10">
        <div className="flex items-center gap-3"><div className="brand-mark"><img src="/static/lexora-logo.png" alt="NyayVault logo" /></div><div><div className="font-black tracking-tight">Nyay<span className="text-mint">Vault</span></div><div className="text-[10px] uppercase tracking-[.24em] text-slate-500">Secure case intelligence</div></div></div>
        <div className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[.04] px-3 py-2 text-xs text-slate-400 md:flex"><span className="h-2 w-2 animate-pulse rounded-full bg-green-400 shadow-[0_0_12px_#4ade80]"/> All systems operational</div>
        <div className="flex items-center gap-2"><button aria-label="Toggle color theme" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="theme-toggle rounded-xl border border-white/10 p-2 text-slate-400 transition hover:border-mint/40 hover:text-mint">{theme === "dark" ? <Sun size={17}/> : <Moon size={17}/>}</button><button onClick={() => setMobileNav(!mobileNav)} className="rounded-lg border border-white/10 p-2 md:hidden"><Menu size={18}/></button><div className="hidden text-right sm:block"><div className="text-sm font-semibold">{user?.full_name || "Secure workspace"}</div><div className="text-xs text-slate-500">{user?.role || "authentication required"}</div></div><button onClick={logout} className="rounded-xl border border-white/10 p-2 text-slate-400 transition hover:border-red-300/40 hover:text-red-300"><LogOut size={17}/></button></div>
      </div>
    </header>
    <div className="mx-auto flex max-w-[1500px]">
      <aside className={`${mobileNav ? "fixed inset-x-4 top-20 z-20" : "hidden"} w-64 shrink-0 rounded-2xl border border-white/10 bg-panel p-3 shadow-2xl md:sticky md:top-24 md:block md:h-[calc(100vh-7rem)] md:border-0 md:bg-transparent md:shadow-none`}>
      ...
      <main className="animate-reveal relative min-w-0 flex-1 px-5 py-8 lg:px-10">
...
function Intelligence({query, onSearch, runSearch, results, searchType, setSearchType, searchLoading}) {
  ...
  return <>  <PageTitle eyebrow="ASK" title="Grounded case intelligence" subtitle="Retrieve authorized evidence first, then analyze it with citations, verification flags and human-review safeguards."/>
    <div className="rounded-3xl border border-electric/20 bg-gradient-to-br from-electric/10 via-panel to-panel p-6 sm:p-10">
      ...
      {recent.length > 0 && <div className="mt-5"><div className="mb-2 text-[10px] uppercase tracking-widest text-slate-600">Recent searches</div><div className="flex flex-wrap gap-2">{recent.map(item => <button key={item} onClick={() => runSearch(item)} className="flex items-center gap-1 rounded-lg bg-white/[.04] px-3 py-2 text-xs text-slate-400 hover:text-white"><Search size={11}/>{item}</button>)}</div></div>}
